...
* **공개 일자:** 2026-09-13 (초안 2026-08-20, v0.2.1 2026-08-22, v3.0 2026-08-22, v3.3 2026-08-23, v3.4 2026-09-13)
...
* **검색 키워드:** CWP, CWP-Entry, 전기차 배터리 스왑, 철도 차량 대차 정렬, 터널 모빌리티 유도, 진입 유도, 위치 정렬, V레일, U레일, 가이드 홈, 무레일 휠 가이드, 롤러 가이드, 라인 레이저, 0.1ms 하드웨어 인터셉트, 최소 개보수 인프라 연동, 콰이어트 어시스트, 페일세이프, 선택 통과, 레일·무레일 모빌리티 정렬, Prior Art
...
* **기존 인프라 최소 개보수 연동:** 고가의 전용 로봇 설비를 새로 구축하거나 바닥을 광범위하게 개보수하는 방식 대신, 전국 주유소, 주차장, 자동 세차장뿐만 아니라 **철도 정비기지, 지하 터널 및 물류 노면의 기존 V레일·체인 이송·안내 레일 메커니즘**을 상위 개념으로 포괄하여 원용. 5센티미터(SLIM) 또는 8센티미터(SHUTTLE) 및 대형 중량형 모듈을 기존 인프라 표면에 최소 개보수 방식으로 연동하여 초기 구축 비용 경감 및 거점화를 도모함.
...
* **공용 기술 결합:** 특정 기업의 독점 기술을 배제하고 표준 기계요소(시소, 차동 기어, V/U/C/T홈 등)를 조합하여 로열티 부담 완화 및 특정 기업의 특허 독점 가능성 완화를 지향함.
...
**본 CWP-Entry와 CWP-Battery-Swap은 별개의 독립 기술이 아니라, 진입 유도(Guide->Entry->V-Home)부터 차동 감속 저충격 도킹까지 하나의 연속 공정으로 연결되도록 설계된 통합 시스템이다.**

* **진입 유도 및 1차 정렬 (`CWP-Entry` - 본 기술):** 세차장 및 철도·터널 공용 V/U레일 인프라 및 지면 가이드 홈 원용과 라인 레이저 가이드를 통해 모빌리티 진입 오차를 완화하고 정비·점검 구역으로 정밀 유도함.
...
* **도면 1 (FIG. 1):** CWP-Entry 자율 유도 및 정렬 시스템의 전체 등각 투영도
  ...
  * V레일/U레일 가이드 채널, 지면 매입 홈 및 체인/견인 메커니즘을 통한 하부 2차 정렬 (철도 대차, 터널 이송 및 무레일 휠 가이드 구조 포괄)
...
### 6.2 전방위 모빌리티 및 궤도·무레일 확장성 (Off-Rail & Railless Extension)
* **형태 규격 독립성 (Form-Factor Agnostic):** 승용 전기차 외 **철도 차량(전동차·기관차 대차 정렬), 터널 내 특수 이송 모빌리티, 도심항공교통/드론 착륙 스키드, 물류 무인운반차(AGV) 및 자율이동로봇(AMR), 마이크로 모빌리티, 특수 탐사 로버, 궤도상 모듈 및 중량 페이로드 정렬 구조**로 확장 가능함.
* **물리 궤도 매체 비한정성:** 본 유도 메커니즘은 돌출형 금속 레일 구조에 한정되지 않으며, 지면 매입형 V/U 가이드 홈(Groove), 비구동 롤러 가이드, 무레일(Railless) 노면 휠 가이드 턱, 자율 이송 광역 가이드 라인 등 물리적·광학적 궤적 안내 수단이 존재하는 모든 진입 유도 구조에 범용으로 포괄 적용됨.
...
* **선택 통과 연동 (레일·무레일 공용화):** 최소 개보수 인프라 연동 원칙에 따라 신규 전용 로봇 인프라 추가 없이 기존 자동 세차장, 철도 입환선, 터널 내 주행 레일 및 물류 가이드 인프라를 상호 원용하여 진출입 동선을 구현함.
  * **스왑/정비 이용 모빌리티:** 작업 완료 후 기존 레일/가이드 라인으로 자연스럽게 진입하여 후속 공정 수행.
